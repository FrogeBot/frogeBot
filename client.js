...
client = new Client({ intents: [Intents.FLAGS.GUILDS, Intents.FLAGS.GUILD_MEMBERS, Intents.FLAGS.GUILD_EMOJIS_AND_STICKERS] });
const fs = require("fs")
...
client.on('ready', () => {
  if(client.shard) {
    console.log = function(text) {
      client.shard.send(["log", text]);
    }
    console.error = function(text) {
      client.shard.send(["error", text]);
    }
    console.info = function(text) {
      client.shard.send(["info", text]);
    }
    console.warn = function(text) {
      client.shard.send(["warn", text]);
    }
    console.debug = function(text) {
      client.shard.send(["debug", text]);
    }
  }
  ...
  // Create web_images directory if required
    if(process.env.WEB_ENABLED == "true" && !client.shard) {
    ...
    if(process.env.WEB_ENABLED == "true" && !client.shard) { // If using web
