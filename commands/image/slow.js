    ...
    try {
        ...
        if(imgUrl.match(/(\.gif)/gi)) { // If type is GIF
            try {
                ...
                worker.postMessage({ imgUrl, list: null, frameSkip: 1, speed: 0.5, options: { imageMagick: process.env.USE_IMAGEMAGICK, maxGifSize: process.env.MAX_GIF_SIZE, maxImageSize: process.env.MAX_IMAGE_SIZE } }) // Send message to GIF worker
