...
async function execute(message, serverQueue, args) {
    try {
        ...
        if (isLinkOrId) {
            ...
            let filterUrl = (await ytsr.getFilters(args))
                .get('Type')
                .get('Video').url;
...
function skip(message, serverQueue) {
    ...
    if (serverQueue.skips >= toSkip) {
        message.channel.send({
            embed: {
                title: `Skipped (${Math.round(
                    (members * sPercent) / 100
                )}/${Math.round((members * sPercent) / 100)})`,
...
function getQueue(message, serverQueue, args) {
    if (!serverQueue || serverQueue.songs.length == 0) {
        ...
        let songsMapped = serverQueue.songs
            .map(
                (s, i) =>
                    `${i + 1}) ${s.title}  |  [${s.duration.durationFormat()}]`
            )
...
function playTrack(guild, song) {
    ...
    try {
        ...
        serverQueue.songs[0].startTime = Math.round(
